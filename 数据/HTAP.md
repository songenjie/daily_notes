...
以[mysql](https://cloud.tencent.com/product/cdb?from=10680)为例，我们来看一下单体数据库的逻辑架构，如下图：

![img](https://ask.qcloudimg.com/http-save/yehe-6973249/4amjw8zjt5.png?imageView2/2/w/1620)

从这个图我们可以看到，mysql包括连接器层，server层，存储引擎层和数据/文件层。单体数据库场景下，数据库本身就是支持事务的，我们不用事务做额外的工作。

如果需要分库分表，这时架构就需要调整，如下图：

![img](https://ask.qcloudimg.com/http-save/yehe-6973249/a7qzid5ktx.png?imageView2/2/w/1620)
...
![img](https://ask.qcloudimg.com/http-save/yehe-6973249/apigdicd8r.png?imageView2/2/w/1620)
...
![img](https://ask.qcloudimg.com/http-save/yehe-6973249/n9winxn9u1.png?imageView2/2/w/1620)
...
TBase是腾讯数据平台团队在基于[PostgreSQL](https://cloud.tencent.com/product/postgresql?from=10680)研发的，支持HTAP(Hybrid Transaction and Analytical Process)，主要由协调节点、数据节点和全局事务管理器(GTM)组成。特点如下：
...
支持 MySQL、PostgreSQL、SparkSQL 和 [MariaDB](https://cloud.tencent.com/product/tdsql?from=10680) 四种关系型数据库实例，100%兼容MySQL语法
